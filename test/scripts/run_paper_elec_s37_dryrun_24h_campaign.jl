...
const _OUT_ROOT = normpath(@__DIR__, "..", "..", "reports", "paper_elec_s_37_campaign_dryrun_24h")
const _RUNS = [
    ("H_24h_gmin_0p0", "elec_s_37_24h_from_0301", 24, 0.0),
    ("H_24h_gmin_1p0", "elec_s_37_24h_from_0301", 24, 1.0),
    ("H_24h_gmin_2p0", "elec_s_37_24h_from_0301", 24, 2.0),
]
const _POSTHOC_BASE_ALPHA = 1.0
...
function _run_one_case(git::Dict{String,Any}, run_name::String, case_dir::String, horizon_hours::Int, g_min::Float64)
    ...
    run_config = Dict(
        ...
        "number_of_weeks" => horizon_hours / 168.0,
        "annual_operation_scaling_factor" => 52.0 / (horizon_hours / 168.0),
        ...
        "solver_name" => "HiGHS",
        "solver_options" => Dict("silent" => true),
        "timestamp" => timestamp,
    ...
    pm = _PM.instantiate_model(
        data,
        _PM.DCPPowerModel,
        pm -> _FP.build_uc_gscr_block_integration(pm; final_storage_policy=:short_horizon_relaxed);
    ...
    if stop_safety
        ...
        return Dict("run_name" => run_name, "horizon_hours" => horizon_hours, "snapshot_count" => snap_count, "g_min" => g_min, "status" => status, "stop_entire_campaign" => true, "stop_reason" => stop_reason)
    end
    ...
    if !(status in _ACTIVE_OK)
        ...
        return Dict("run_name" => run_name, "horizon_hours" => horizon_hours, "snapshot_count" => snap_count, "g_min" => g_min, "status" => status, "stop_entire_campaign" => false)
    end
    ...
    posthoc_summary, posthoc_ts, node_binding = _posthoc_strength(pm, g_min)
    ...
    _write_csv(
        ...
        ["snapshot", "mu_t", "gSCR_t", "relative_margin_rG", "min_kappa", "min_kappa_bus", "binding_node_count", "local_relative_slack_min", "gershgorin_conservatism_gap"],
        [[r["snapshot"], r["mu_t"], r["gSCR_t"], r["relative_margin_rG"], r["min_kappa"], r["min_kappa_bus"], r["binding_node_count"], r["local_relative_slack_min"], r["gershgorin_conservatism_gap"]] for r in posthoc_ts],
    )
    _write_csv(
        normpath(out_dir, "node_binding_frequency.csv"),
        ["bus", "region_name", "beta_i", "gfm_bess_capacity_GW", "min_kappa", "mean_kappa", "p05_kappa"],
        [[r["bus"], r["region_name"], r["beta_i"], r["gfm_bess_capacity_GW"], r["min_kappa"], r["mean_kappa"], r["p05_kappa"]] for r in node_binding],
    )
...
function _campaign_outputs(rows::Vector{Dict{String,Any}})
    _write_json(normpath(_OUT_ROOT, "campaign_summary.json"), Dict("generated_at" => Dates.format(now(), "yyyy-mm-ddTHH:MM:SS"), "rows" => rows))
    header = [
        "run_name", "horizon_hours", "snapshot_count", "g_min", "status", "objective", "solve_time", "relative_gap",
        "total_annual_system_cost_BEUR_per_year", "installed_BESS_GFM_GW", "installed_BESS_GFL_GW", "installed_RES_GW", "installed_CCGT_GW",
        ...
        "min_mu_t", "min_gSCR_t", "min_gSCR_margin", "weakest_bus", "weakest_snapshot", "near_binding_count", "max_AC_branch_loading",
    ...
    for r in rows
        push!(csv_rows, [
            r["run_name"], r["horizon_hours"], r["snapshot_count"], r["g_min"], r["status"], get(r, "objective", nothing), get(r, "solve_time", nothing), get(r, "relative_gap", nothing),
            get(r, "total_annual_system_cost_BEUR_per_year", nothing), get(r, "installed_BESS_GFM_GW", nothing), get(r, "installed_BESS_GFL_GW", nothing), get(r, "installed_RES_GW", nothing), get(r, "installed_CCGT_GW", nothing),
            ...
            get(r, "min_mu_t", nothing), get(r, "min_gSCR_t", nothing), get(r, "min_gSCR_margin", nothing), get(r, "weakest_bus", nothing), get(r, "weakest_snapshot", nothing), get(r, "near_binding_count", nothing), get(r, "max_AC_branch_loading", nothing),
    ...
    _write_csv(
        ...
        ["scenario", "horizon_hours", "g_min", "total_annual_system_cost_BEUR_per_year", "installed_BESS_GFM_GW", "installed_BESS_GFL_GW", "installed_RES_GW", "installed_CCGT_GW", "solver_wall_clock_min", "min_mu_t", "min_gSCR_t"],
        [[r["run_name"], r["horizon_hours"], r["g_min"], get(r, "total_annual_system_cost_BEUR_per_year", nothing), get(r, "installed_BESS_GFM_GW", nothing), get(r, "installed_BESS_GFL_GW", nothing), get(r, "installed_RES_GW", nothing), get(r, "installed_CCGT_GW", nothing), get(r, "solve_time", 0.0) / 60.0, get(r, "min_mu_t", nothing), get(r, "min_gSCR_t", nothing)] for r in rows],
    )
    ...
    for r in rows
        ...
        if isfile(node_path)
            ...
            for i in axes(data, 1)
                ...
                lat = ""
                lon = ""
                push!(map_rows, [r["run_name"], bus, region, data[i, col["beta_i"]], data[i, col["gfm_bess_capacity_GW"]], lat, lon])
            end
        end
        if isfile(ts_path)
            ...
            col = Dict(String(hdr[i]) => i for i in eachindex(hdr))
            for i in axes(data, 1)
                push!(dist_rows, [r["run_name"], data[i, col["snapshot"]], data[i, col["mu_t"]], data[i, col["gSCR_t"]], data[i, col["relative_margin_rG"]], data[i, col["local_relative_slack_min"]], data[i, col["gershgorin_conservatism_gap"]]])
            end
        end
    end
    _write_csv(normpath(_OUT_ROOT, "paper_figure_1_spatial_map.csv"), ["scenario", "bus", "region_name", "beta_i", "gfm_bess_capacity_GW", "latitude", "longitude"], map_rows)
    _write_csv(normpath(_OUT_ROOT, "paper_figure_2_distribution.csv"), ["scenario", "snapshot", "mu_t", "gSCR_t", "relative_margin_rG", "local_relative_slack_min", "gershgorin_conservatism_gap"], dist_rows)
    ...
    open(normpath(_OUT_ROOT, "campaign_summary.md"), "w") do io
        println(io, "# Paper elec_s_37 24h Dry-Run CAPEXP/gSCR Campaign")
        println(io)
        ...
        println(io, "## Runs")
        for r in rows
            println(io, "- ", r["run_name"], ": status=", r["status"], ", objective=", _fmt(get(r, "objective", nothing)), ", min_gSCR_margin=", _fmt(get(r, "min_gSCR_margin", nothing)))
        end
        println(io)
        println(io, "## 24h Technical Matrix")
        for r in rows
            println(io, "- g_min=", r["g_min"], ": total_annual_system_cost_BEUR_per_year=", _fmt(get(r, "total_annual_system_cost_BEUR_per_year", nothing)), ", BESS-GFM_GW=", _fmt(get(r, "installed_BESS_GFM_GW", nothing)), ", min_mu_t=", _fmt(get(r, "min_mu_t", nothing)), ", min_gSCR_t=", _fmt(get(r, "min_gSCR_t", nothing)))
        end
    end
end
...
function main()
    ...
    for d in ["elec_s_37_24h_from_0301"]
    ...
    for (run_name, case_dir, hours, gmin) in _RUNS
        if stop_campaign
            break
        end

        row = _run_one_case(git, run_name, case_dir, hours, gmin)
        push!(rows, row)
    ...
    if stage_g0_failed
        error("Stopped after 24h g_min=0 feasibility failure by policy.")
    end
end
