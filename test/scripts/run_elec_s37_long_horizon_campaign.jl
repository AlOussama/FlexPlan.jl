...
const _OUT_ROOT = normpath(@__DIR__, "..", "..", "reports", "long_horizon_elec_s_37_campaign")
const _RUNS = [
    ("H_1week_gmin_0p0", "elec_s_37_1week_from_0301", 1, 0.0),
    ("H_1week_gmin_1p0", "elec_s_37_1week_from_0301", 1, 1.0),
    ("H_1week_gmin_1p5", "elec_s_37_1week_from_0301", 1, 1.5),
    ("H_1week_gmin_2p0", "elec_s_37_1week_from_0301", 1, 2.0),
    ("H_2week_gmin_0p0", "elec_s_37_2weeks_from_0301", 2, 0.0),
    ("H_2week_gmin_1p0", "elec_s_37_2weeks_from_0301", 2, 1.0),
    ("H_2week_gmin_1p5", "elec_s_37_2weeks_from_0301", 2, 1.5),
    ("H_2week_gmin_2p0", "elec_s_37_2weeks_from_0301", 2, 2.0),
]
    ...
    for nw in values(data["nw"])
        for table in ("gen", "storage", "ne_storage")
            for d in values(get(nw, table, Dict{String,Any}()))
                if haskey(d, "cost_inv_block")
                    d["cost_inv_block"] = float(d["cost_inv_block"]) * factor
                end
            end
        end
    end
...
function _run_one_case(git::Dict{String,Any}, run_name::String, case_dir::String, horizon_weeks::Int, g_min::Float64)
    ...
    run_config = Dict(
        ...
        "horizon_length" => "$(horizon_weeks) week",
    ...
    if stop_safety
        ...
        return Dict("run_name" => run_name, "horizon_weeks" => horizon_weeks, "snapshot_count" => snap_count, "g_min" => g_min, "status" => status, "stop_entire_campaign" => true, "stop_reason" => stop_reason)
    end
    ...
    if !(status in _ACTIVE_OK)
        ...
        return Dict("run_name" => run_name, "horizon_weeks" => horizon_weeks, "snapshot_count" => snap_count, "g_min" => g_min, "status" => status, "stop_entire_campaign" => false)
    end
    ...
    _write_csv(
        ...
        ["carrier", "invested_blocks"],
        [[k, v] for (k, v) in sort(collect(metrics["investment_by_carrier"]); by=first)],
    )
    _write_csv(
        normpath(out_dir, "investment_by_bus.csv"),
        ["bus", "invested_blocks"],
        [[k, v] for (k, v) in sort(collect(metrics["investment_by_bus"]); by=first)],
    )
    _write_csv(
    ...
    open(normpath(out_dir, "run_report.md"), "w") do io
        ...
        println(io)
        println(io, "- status: ", status)
        println(io, "- objective: ", metrics["objective"])
...
function _campaign_outputs(rows::Vector{Dict{String,Any}})
    ...
    header = [
        "run_name", "horizon", "snapshot_count", "g_min", "status", "objective", "solve_time", "relative_gap",
        ...
        "min_gSCR_margin", "weakest_bus", "weakest_snapshot", "near_binding_count", "max_AC_branch_loading",
    ...
    for r in rows
        push!(csv_rows, [
            r["run_name"], r["horizon_weeks"], r["snapshot_count"], r["g_min"], r["status"], get(r, "objective", nothing), get(r, "solve_time", nothing), get(r, "relative_gap", nothing),
            ...
            get(r, "min_gSCR_margin", nothing), get(r, "weakest_bus", nothing), get(r, "weakest_snapshot", nothing), get(r, "near_binding_count", nothing), get(r, "max_AC_branch_loading", nothing),
    ...
    open(normpath(_OUT_ROOT, "campaign_summary.md"), "w") do io
        println(io, "# Long-Horizon elec_s_37 CAPEXP/gSCR Campaign")
        println(io)
        ...
        println(io)
        println(io, "## 1-Week Investment vs g_min")
        for r in filter(x -> x["horizon_weeks"] == 1, rows)
            println(io, "- g_min=", r["g_min"], ": total_invested_blocks=", _fmt(get(r, "total_invested_blocks", nothing)), ", battery_gfm_blocks=", _fmt(get(r, "battery_gfm_blocks", nothing)), ", min_gSCR_margin=", _fmt(get(r, "min_gSCR_margin", nothing)))
        end
        println(io)
        println(io, "## 2-Week Investment vs g_min")
        for r in filter(x -> x["horizon_weeks"] == 2, rows)
            println(io, "- g_min=", r["g_min"], ": total_invested_blocks=", _fmt(get(r, "total_invested_blocks", nothing)), ", battery_gfm_blocks=", _fmt(get(r, "battery_gfm_blocks", nothing)), ", min_gSCR_margin=", _fmt(get(r, "min_gSCR_margin", nothing)))
        end
    end
end
...
function main()
    ...
    for d in ["elec_s_37_1week_from_0301", "elec_s_37_2weeks_from_0301"]
    ...
    rows = Dict{String,Any}[]
    stop_campaign = false
    stage1_g0_failed = false
    stage2_g0_failed = false

    for (run_name, case_dir, weeks, gmin) in _RUNS
        if stop_campaign
            break
        end
        if weeks == 2 && length(filter(r -> r["horizon_weeks"] == 1, rows)) < 4
            error("Two-week stage attempted before all one-week runs were completed.")
        end

        row = _run_one_case(git, run_name, case_dir, weeks, gmin)
        push!(rows, row)
        ...
        if weeks == 1 && gmin == 0.0 && !(row["status"] in _ACTIVE_OK)
            stage1_g0_failed = true
            break
        end
        if weeks == 2 && gmin == 0.0 && !(row["status"] in _ACTIVE_OK)
            stage2_g0_failed = true
            break
        end
    end
    ...
    if stage1_g0_failed
        error("Stopped after 1-week g_min=0 feasibility failure by policy.")
    end
    if stage2_g0_failed
        error("Stopped after 2-week g_min=0 feasibility failure by policy.")
    end
end
