...
const _EPS = 1e-6
const _NEAR = 1e-6
const _INVESTMENT_COST_SCALE_24H = 1.0
    ...
    for nw in values(data["nw"])
        ...
        for gen in values(get(nw, "gen", Dict{String,Any}()))
            ...
            if haskey(gen, "cost_inv_block")
                gen["cost_inv_block"] = float(get(gen, "cost_inv_block", 0.0))
            end
        end
        for table in ("storage", "ne_storage")
            for st in values(get(nw, table, Dict{String,Any}()))
                ...
                if haskey(st, "cost_inv_block")
                    st["cost_inv_block"] = float(get(st, "cost_inv_block", 0.0))
                end
                st["r"] = get(st, "r", 0.0)
                st["x"] = get(st, "x", 0.0)
...
function _write_report(
    ...
    open(_REPORT_PATH, "w") do io
        ...
        println(io, "Investment-cost policy: annualized `cost_inv_block` is used directly; no solver-copy day/week scaling is applied.")
        println(io)
        ...
        println(io, "- all candidate cost_inv_block positive: ", vis["cost_inv_block_scaled_positive_all_candidates"])
        ...
        println(io, "- all candidate investment coefficients are positive: ", vis["cost_inv_block_scaled_positive_all_candidates"])
